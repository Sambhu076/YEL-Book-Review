...
# --- PERMANENT, CORRECTED FUNCTION ---
def livekit_token(request):
    # This key and secret MUST match your LiveKit project
    api_key = os.getenv("LIVEKIT_API_KEY")
    api_secret = os.getenv("LIVEKIT_API_SECRET")

    # Check if the keys are loaded from your .env file
    if not api_key or not api_secret:
        return JsonResponse({'error': 'LiveKit API key or secret not set in .env file.'}, status=500)

    # Create the claims for the JWT
    claims = {
        'exp': int(time.time()) + (10 * 60),  # Token expires in 10 minutes
        'nbf': int(time.time()) - 60,         # Token is valid starting 1 minute ago
        'iss': api_key,                       # Issuer is the API Key
        'sub': 'student-user',                # Subject is the user's identity
        'name': 'Student',                    # User's display name
        'video': {
            'roomJoin': True,
            'room': 'book-review-room',
            'canPublish': True,
            'canSubscribe': True,
        }
    }

    # Encode the token with your API Secret
    token = jwt.encode(
        claims,
        api_secret,
        algorithm='HS256'
...
def check_question1_answer(request):
    """
    API endpoint to check Question 1 answer - Story Title (Text Input).
    This now calls the refactored logic.
    """
    try:
        ...
        result_dict = openrouter_logic.analyze_title_answer(user_answer)
...
# ... Update all other check_questionX_answer views similarly ...
# They will continue to work for typed text submissions.
...
#     "misspelled_words": [["list", "for", "answer1"], ["list", "for", "answer2"], ["list", "for", "answer3"]]
# }}
...
- Only include words that are clearly misspelled. Do not include proper nouns.
- If there are no spelling mistakes, return an empty list: [].
- They should be passed on through "message".
...
def create_story_events_fallback_response(user_answers):
    ...
    if isinstance(user_answers, list):
        ...
        misspelled_words = [[], [], []]  # Array of arrays for text submission
    else:
        all_text = user_answers.lower()
        misspelled_words = []  # Simple array for voice submission
...
- **Sentence Starters Check:** The student was prompted to use "My favourite character is" and "I like them because". If they use these starters correctly, it contributes to an "excellent" or "good" rating. **If they give a good answer but do not use the starters, your `message` should gently remind them to use the full sentence starters next time.**
...
def check_question8_answer(request):
    """
    API endpoint to check Question 8 answer - Favourite part of the story.
    """
    try:
        data = json.loads(request.body)
        user_answer = data.get('answer', '').strip()
        ...
        if not user_answer:
            return JsonResponse({
                'error': 'Please tell us your favourite part and why you liked it.'
            }, status=400)

        # Basic validation checks remain useful
        if len(user_answer) < 10:
            return JsonResponse({
                'isCorrect': False,
                'message': 'That\'s a start! Can you write a full sentence about which part you liked and why?',
                'feedback_type': 'guidance',
                'show_answer': False
            })
        ...
        if not user_answer[0].isupper():
            return JsonResponse({
                'isCorrect': False,
                'message': 'Great thought! Just remember to start your sentence with a capital letter.',
                'feedback_type': 'correction',
                'show_answer': False,
                'highlight_issue': 'capitalization'
            })
        ...
        # --- MODIFIED --- 
        # Call the new, correctly-named function for this question's logic
...
def analyze_favorite_part_answer(user_answer):
    """
    Use AI to analyze the user's answer about their favorite part of the story.
    """
    api_key = os.getenv('OPENROUTER_API_KEY2')
    if not api_key:
        # Use the new fallback if the API key is missing
    ...
    headers = {
        ...
        "X-Title": "Story Favorite Part Checker" # Replace with your app's title
    }

    # --- UPDATED PROMPT ---
    # The prompt is completely rewritten to evaluate the new question.
    prompt = f"""You are a friendly and encouraging reading assistant for a young student. You are evaluating their answer to the question: "Which is your favourite part of the story 'Goldilocks and the Three Bears', and why?"

Your task is twofold:
1.  Evaluate if the student identified a specific part of the story and provided a reason for why they liked it.
2.  Identify any misspelled English words in their answer.

Evaluation Guidelines:
-   **Sentence Starters Check:** The student was prompted to use "My favourite part is..." and "I liked it because...". If they use these or similar phrasing, this contributes positively to an "excellent" or "good" rating. **If they provide a good answer but omit the starters, your `message` SHOULD include a gentle reminder to use full sentences to make their answer clearer next time.**
-   **Excellent**: The student clearly names a part of the story (e.g., trying the porridge, breaking the chair, the bears coming home) AND gives a clear reason (e.g., "because it was funny," "because she found the perfect bed").
-   **Good**: The student names a part of the story and gives a simple, but valid, reason.
-   **Partial**: The student names a part of the story but gives NO reason, OR gives a vague answer that is hard to connect to a specific event. The feedback should prompt them for the "why".
-   **Needs Improvement**: The answer is off-topic, nonsensical, or doesn't mention anything from the story.
...
Notes:
-   Since this is an opinion question, an answer is "correct" (isCorrect: true) if it's rated "excellent", "good", or "partial". It is only "incorrect" (isCorrect: false) if it "needs_improvement".
-   For this question, `show_answer` must ALWAYS be `false`.
-   If there are no spelling mistakes, return an empty list for "misspelled_words": [].
...
Student's answer: "{user_answer}" """
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": prompt},
            {"role": "user", "content": f'Please analyze this answer about a favorite part of the story: "{user_answer}"'}
        ],
        "temperature": 0.4,
        "max_tokens": 250
    }
    ...
    try:
        ...
        # If the API call fails for other reasons (e.g., 500 error from their side)
        return create_favorite_part_fallback_response(user_answer)

    except requests.RequestException:
        # If the request itself fails (e.g., no internet connection)
        return create_favorite_part_fallback_response(user_answer)
...
def create_favorite_part_fallback_response(user_answer):
    """
    Create a fallback response for the favorite part question if the AI service fails.
    This uses keywords to check if the answer is relevant.
    """
    user_lower = user_answer.lower()
    
    # Keywords related to specific events in the story
    story_part_keywords = ['porridge', 'chair', 'bed', 'bears', 'house', 'cottage', 
                           'tasting', 'sitting', 'sleeping', 'running', 'woke', 'ate', 'broke']
    
    # Keywords that indicate reasoning or preference
    reasoning_keywords = ['because', 'liked', 'favourite', 'funny', 'scary', 
                          'interesting', 'my favorite', 'i like']

    has_story_part = any(word in user_lower for word in story_part_keywords)
    has_reasoning = any(word in user_lower for word in reasoning_keywords)

    if has_story_part and has_reasoning:
        return JsonResponse({
            'isCorrect': True,
            'message': 'That\'s a great choice! Thanks for explaining why you liked that part.',
            'feedback_type': 'good',
            'show_answer': False,
            'misspelled_words': []
        })
        ...
        return JsonResponse({
            'isCorrect': True,
            'message': 'Good pick! Can you also tell me *why* that was your favourite part?',
            'feedback_type': 'partial',
            'show_answer': False,
            'misspelled_words': []
        })
        ...
        return JsonResponse({
            'isCorrect': False,
            'message': 'Try to think about a part of the story, like when Goldilocks tried the porridge, the chairs, or the beds. Then, tell me which one you liked best.',
            'feedback_type': 'needs_improvement',
            'show_answer': False,
            'misspelled_words': []
        })