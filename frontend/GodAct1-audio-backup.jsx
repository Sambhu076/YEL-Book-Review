...
export default function GodAct1() {
  ...
  // --- NEW STATE to manage the two-part workflow
  ...
  // --- CORE AUTOMATION LOGIC for the voice interaction
  useEffect(() => {
    // ADD THIS LINE to ensure a clean start every time.
    resetTranscript();

    if (!browserSupportsSpeechRecognition) {
      alert("This browser doesn't support speech recognition. Please try Chrome.");
      return;
    }
    ...
    // When the audio finishes, update the state
  ...
  }, [browserSupportsSpeechRecognition, resetTranscript]); // Added resetTranscript to dependency array



  // Effect 2: Starts listening AFTER the question is finished
  useEffect(() => {
    if (isQuestionFinished && !listening) {
      console.log("DEBUG: State updated. Starting to listen...");
      SpeechRecognition.startListening({ continuous: false, language: 'en-US' });
    }
  }, [isQuestionFinished, listening]);

  // 3. When the user stops talking, auto-submit their answer.
  useEffect(() => {
    if (finalTranscript) {
  ...
  // --- AUDIO & SPEECH SYNTHESIS FUNCTIONS (Your existing functions are preserved)
  const stopSpeaking = () => {
    if (currentAudio) {
      currentAudio.pause();
  ...
  const speakText = (text, onEndCallback = () => { }) => {
    stopSpeaking();
    // Your comprehensive speakText, speakWithElevenLabs, and speakWithBrowserSynthesis functions remain here...
    // For brevity, they are condensed. The logic is the same as in your provided file.
    setIsSpeaking(true);
  ...
  const submitAnswer = async (answer, submissionType) => {
    ...
    try {
      ...
      if (submissionType === 'voice') {
        // 4. For voice answers, give audio-only feedback.
        speakText(data.message, () => {
          // 5. After feedback, play the next prompt and switch to text input mode.
          const nextPromptAudio = new Audio('/abc.mp3');
          nextPromptAudio.play();
          nextPromptAudio.onended = () => {
            setInteractionStage('text');
          };
        });
      } else { // 'text' submission
        // For text answers, show text and give audio feedback.
        setFeedback(data);
      ...
      const errorFeedback = { message: 'An error occurred. Please try again.', isCorrect: false };
      if (submissionType === 'text') {
        setFeedback(errorFeedback);
      }
      // If voice submission fails, switch to text mode so the user isn't stuck.
      setInteractionStage('text');
    } finally {
      setIsLoading(false);
      resetTranscript();
  ...
  // --- HANDLER FUNCTIONS for the text input stage
  ...
  return (
    <div style={{
      ...
      <div className="banner-section">
        ...
        {showPlayButton && (
          <button className="audio-play-button" onClick={() => new Audio('/title.mp3').play()}>
            <FaPlay />
          </button>
        )}
      </div>
            ...
            <div className="voice-input-section" style={{ textAlign: 'center', padding: '40px', background: 'white', borderRadius: '20px', boxShadow: '0 8px 25px rgba(0,0,0,0.1)' }}>
              {listening ? (
                <>
                  ...
                  <p style={{ fontFamily: "'Sen', sans-serif", fontSize: '20px', color: '#333' }}>Listening...</p>
                  <p style={{ fontFamily: "'Segoe UI'", fontSize: '16px', color: '#666', minHeight: '50px' }}><em>{transcript}</em></p>
                </>
              ) : (
                <>
                  <FaMicrophoneSlash size={50} color="#6c757d" style={{ marginBottom: '20px' }} />
                  <p style={{ fontFamily: "'Sen', sans-serif", fontSize: '20px', color: '#333' }}>
                    {isLoading ? 'Checking answer...' : 'Say your answer'}
                  </p>
                  <p style={{ fontFamily: "'Segoe UI'", fontSize: '16px', color: '#666', minHeight: '50px' }}>&nbsp;</p>
                </>
              )}
            </div>
          ) : (
            // STAGE 2: TEXT INPUT INTERFACE (Your Client-Approved UI)
            <>
