...
export default function PetAct13() {
  ...
  useEffect(() => {
    const playFeelingsAudio = () => {
      try {
        const audio = new Audio('/feel.mp3');
        audio.volume = 0.7;
        
        audio.addEventListener('loadeddata', () => {
          console.log('Peter Rabbit feelings audio loaded successfully');
        });
        
        audio.addEventListener('ended', () => {
          console.log('Peter Rabbit feelings audio playback completed');
          setAudioPlayed(true);
        });
        
        audio.addEventListener('error', (e) => {
          console.error('Peter Rabbit feelings audio error:', e);
        ...
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              console.log('Peter Rabbit feelings audio started playing automatically');
        ...
        console.error('Error setting up Peter Rabbit feelings audio:', error);
        setShowPlayButton(true);
        return null;
      }
    };
  ...
  const handleManualPlay = () => {
    try {
      ...
      audio.play().then(() => {
        setShowPlayButton(false);
        console.log('Peter Rabbit feelings audio started playing manually');
      }).catch((error) => {
  ...
  const submitAnswer = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/check-peter-question13/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          answer: feelingsAnswer.trim()
        })
      });
    ...
    <div style={{
      ...
      <div className="banner-section">
        ...
        <div className="banner-content">
          <h1 className="banner-title">Reading Reflection</h1>
        </div>
          ...
          <div className="question-section">
            <div className="answer-field">
              ...
              <input
                ...
                value={feelingsAnswer}
                onChange={(e) => setFeelingsAnswer(e.target.value)}
                disabled={isLoading}
              />
            </div>
          </div>
          ...
          {feedback && (
            <div className={`feedback-section ${feedback.isCorrect ? 'correct' : 'incorrect'}`}>
              ...
              <div className={`feedback-title ${feedback.isCorrect ? 'correct' : 'incorrect'}`}>
                {feedback.isCorrect ? '✓ Great sharing!' : '✗ Try again'}
              </div>
          ...
          <div className="button-section">
            {!feedback ? (
              <button 
                ...
                disabled={!feelingsAnswer.trim() || isLoading}
              >
                {isLoading ? (
                  <>
