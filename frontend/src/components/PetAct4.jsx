...
export default function PetAct4() {
  ...
  useEffect(() => {
    const playAnimalAudio = () => {
      try {
        const audio = new Audio('/1main.mp3');
        audio.volume = 0.7;
        
        audio.addEventListener('loadeddata', () => {
          console.log('Peter Rabbit animal audio loaded successfully');
        });
        
        audio.addEventListener('ended', () => {
          console.log('Peter Rabbit animal audio playback completed');
          setAudioPlayed(true);
        });
        
        audio.addEventListener('error', (e) => {
          console.error('Peter Rabbit animal audio error:', e);
        ...
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              console.log('Peter Rabbit animal audio started playing automatically');
        ...
        console.error('Error setting up Peter Rabbit animal audio:', error);
        setShowPlayButton(true);
        return null;
      }
    };
  ...
  const handleManualPlay = () => {
    try {
      ...
      audio.play().then(() => {
        setShowPlayButton(false);
        console.log('Peter Rabbit animal audio started playing manually');
      }).catch((error) => {
  ...
  const submitAnswer = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/check-peter-question4/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <div className="question-section">
            <div className="answer-field">
              <div className="title-row">
                <label className="field-label">Main Animal 1</label>
                <input
                  type="text"
