...
export default function PetAct2() {
  ...
  useEffect(() => {
    const playAuthorAudio = () => {
      try {
        const audio = new Audio('/author.mp3');
        audio.volume = 0.7;
        
        audio.addEventListener('loadeddata', () => {
          console.log('Peter Rabbit author audio loaded successfully');
        });
        
        audio.addEventListener('ended', () => {
          console.log('Peter Rabbit author audio playback completed');
          setAudioPlayed(true);
        });
        
        audio.addEventListener('error', (e) => {
          console.error('Peter Rabbit author audio error:', e);
        ...
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              console.log('Peter Rabbit author audio started playing automatically');
        ...
        console.error('Error setting up Peter Rabbit author audio:', error);
        setShowPlayButton(true);
        return null;
      }
    };
  ...
  const handleManualPlay = () => {
    try {
      ...
      audio.play().then(() => {
        setShowPlayButton(false);
        console.log('Peter Rabbit author audio started playing manually');
      }).catch((error) => {
          ...
          <div className="button-section">
            {!feedback ? (
              <button 
                ...
                disabled={!authorAnswer.trim() || isLoading}
              >
                {isLoading ? (
                  <>
