...
export default function PetAct6() {
  ...
  useEffect(() => {
    const playIntroAudio = () => {
      try {
        // Correctly set to play the audio for question 6
        const audio = new Audio('/2main.mp3'); 
        audio.volume = 0.7;
        
        audio.addEventListener('loadeddata', () => {
          console.log('Peter Rabbit question 6 audio loaded successfully');
        });
        
        audio.addEventListener('ended', () => {
          console.log('Peter Rabbit question 6 audio playback completed');
          setAudioPlayed(true);
        });
        
        audio.addEventListener('error', (e) => {
          console.error('Peter Rabbit question 6 audio error:', e);
        ...
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              console.log('Peter Rabbit question 6 audio started playing automatically');
        ...
        console.error('Error setting up Peter Rabbit question 6 audio:', error);
        setShowPlayButton(true);
        return null;
      }
    };
  ...
  const handleManualPlay = () => {
    try {
      ...
      audio.play().then(() => {
        setShowPlayButton(false);
        console.log('Peter Rabbit question 6 audio started playing manually');
      }).catch((error) => {
  ...
  const submitAnswer = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/check-peter-question6/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          answer: animalAnswer.trim()
        })
      });
          ...
          <div className="question-section">
            <div className="answer-field">
              <div className="title-row">
                <label className="field-label">Main Animal 2</label>
                <input
                  type="text"
          ...
          <div className="button-section">
            {!feedback ? (
              <button 
                ...
                disabled={!animalAnswer.trim() || isLoading}
              >
                {isLoading ? (
                  <>
