...
export default function PetAct5() {
  ...
  useEffect(() => {
    const playPersonalityAudio = () => {
      try {
        const audio = new Audio('/p1.mp3');
        audio.volume = 0.7;
        
        audio.addEventListener('loadeddata', () => {
          console.log('Peter Rabbit personality audio loaded successfully');
        });
        
        audio.addEventListener('ended', () => {
          console.log('Peter Rabbit personality audio playback completed');
          setAudioPlayed(true);
        });
        
        audio.addEventListener('error', (e) => {
          console.error('Peter Rabbit personality audio error:', e);
        ...
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              console.log('Peter Rabbit personality audio started playing automatically');
        ...
        console.error('Error setting up Peter Rabbit personality audio:', error);
        setShowPlayButton(true);
        return null;
      }
    };
  ...
  const handleManualPlay = () => {
    try {
      ...
      audio.play().then(() => {
        setShowPlayButton(false);
        console.log('Peter Rabbit personality audio started playing manually');
      }).catch((error) => {
  ...
  const submitAnswer = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/check-peter-question5/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
