...
export default function PetAct11() {
  ...
  useEffect(() => {
    const playIntroAudio = () => {
      try {
        // Correctly set to play the audio for question 11
        const audio = new Audio('/les.mp3'); 
        audio.volume = 0.7;
        
        audio.addEventListener('loadeddata', () => {
          console.log('Peter Rabbit question 11 audio loaded successfully');
        });
        
        audio.addEventListener('ended', () => {
          console.log('Peter Rabbit question 11 audio playback completed');
          setAudioPlayed(true);
        });
        
        audio.addEventListener('error', (e) => {
          console.error('Peter Rabbit question 11 audio error:', e);
        ...
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              console.log('Peter Rabbit question 11 audio started playing automatically');
        ...
        console.error('Error setting up Peter Rabbit question 11 audio:', error);
        setShowPlayButton(true);
        return null;
      }
    };
  ...
  const handleManualPlay = () => {
    try {
      ...
      audio.play().then(() => {
        setShowPlayButton(false);
        console.log('Peter Rabbit question 11 audio started playing manually');
      }).catch((error) => {
  ...
  const submitAnswer = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/check-peter-question11/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          answer: lessonAnswer.trim()
        })
      });
      ...
      <Header />
       ...
       <div className="banner-section">
          ...
          />
             ...
             <h1 className="banner-title">The Lesson Learned</h1>
           </div>
            ...
            <div className="button-section">
              {!feedback ? (
                <button 
                  ...
                  disabled={!lessonAnswer.trim() || isLoading}
                >
                  {isLoading ? (
                    <>
