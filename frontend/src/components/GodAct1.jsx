...
export default function GodAct1() {
  ...
  // --- HANDLER TO START THE ACTIVITY ON USER CLICK ---
  ...
  useEffect(() => {
    // ADDED 'interactionStage' check to prevent the mic from restarting.
  ...
  }, [activityStarted, isQuestionFinished, listening, interactionStage]); // Add interactionStage to dependency array

  useEffect(() => {
    if (finalTranscript && !isLoading) {
      console.log("DEBUG: Original transcript received:", finalTranscript);

      // FIX 1: Capitalize the first letter of the voice input.
  ...
  // --- AUDIO & HELPER FUNCTIONS (Correctly Implemented) ---
  const stopSpeaking = () => {
    if (currentAudio) {
      currentAudio.pause();
  ...
  // --- DYNAMIC SUBMIT ANSWER FUNCTION (CORRECTED) ---
  const submitAnswer = async (answer, submissionType) => {
    ...
    try {
      ...
      if (!response.ok || data.error) {
        const errorMessage = data.error || data.message || 'An unknown error occurred. Please try again.';
        throw new Error(errorMessage);
      }

      if (submissionType === 'voice') {
        speakText(data.message, () => {
          // FIX 2: Make the UI transition reliable and not dependent on the next audio.
          console.log("DEBUG: Feedback speech finished. Switching to text input stage.");
          setInteractionStage('text');

          // Attempt to play the prompt audio, but the UI will not wait for it.
          const nextPromptAudio = new Audio('/input_audio.mp3');
          nextPromptAudio.play().catch(e => console.error("Prompt audio ('/input_audio.mp3') failed to play. This is non-critical.", e));
        });
    ...
    <div style={{
      ...
      <div className="main-content">
        ...
        <div className="content-with-button">
          ...
          {/* --- CONDITIONAL UI: SHOW START BUTTON OR ACTIVITY --- */}
          {!activityStarted ? (
            // UI before the activity starts
            ...
            // UI after the activity has started
            <>
