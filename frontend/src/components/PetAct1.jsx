...
export function PetAct1() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(playTitleAudio, 800);
  ...
  const handleManualPlay = () => {
    try {
      const audio = new Audio('/title.mp3');
      audio.volume = 0.7;
      audio.addEventListener('ended', () => setShowPlayButton(false));
  ...
  const submitAnswer = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/check-peter-question1/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
