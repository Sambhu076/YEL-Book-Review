...
export default function PetAct14() {
  const navigate = useNavigate();
  const [currentQuestion, setCurrentQuestion] = useState(14);
  const [storyPartAnswer, setStoryPartAnswer] = useState('');
  ...
  useEffect(() => {
    const playStoryPartAudio = () => {
      try {
        const audio = new Audio('/patr.mp3');
        audio.volume = 0.7;
        
        audio.addEventListener('loadeddata', () => {
          console.log('Peter Rabbit story part audio loaded successfully');
        });
        
        audio.addEventListener('ended', () => {
          console.log('Peter Rabbit story part audio playback completed');
          setAudioPlayed(true);
        });
        
        audio.addEventListener('error', (e) => {
          console.error('Peter Rabbit story part audio error:', e);
        ...
        if (playPromise !== undefined) {
          playPromise
            .then(() => {
              console.log('Peter Rabbit story part audio started playing automatically');
        ...
        console.error('Error setting up Peter Rabbit story part audio:', error);
        setShowPlayButton(true);
        return null;
      }
    };
  ...
  const handleManualPlay = () => {
    try {
      ...
      audio.play().then(() => {
        setShowPlayButton(false);
        console.log('Peter Rabbit story part audio started playing manually');
      }).catch((error) => {
  ...
  const submitAnswer = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/check-peter-question14/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          answer: storyPartAnswer.trim()
        })
      });
    ...
    <div style={{
      ...
      <div className="banner-section">
        ...
        <div className="banner-content">
          <h1 className="banner-title">Story Analysis</h1>
        </div>
          ...
          <div className="question-section">
            <div className="answer-field">
              <div className="field-label">
                Which part made you feel that way?
              </div>
              <input
                className="answer-input"
                placeholder="type answer here"
                value={storyPartAnswer}
                onChange={(e) => setStoryPartAnswer(e.target.value)}
                disabled={isLoading}
              />
            </div>
          </div>
          ...
          {feedback && (
            <div className={`feedback-section ${feedback.isCorrect ? 'correct' : 'incorrect'}`}>
              ...
              <div className={`feedback-title ${feedback.isCorrect ? 'correct' : 'incorrect'}`}>
                {feedback.isCorrect ? '✓ Great connection!' : '✗ Try again'}
              </div>
          ...
          <div className="button-section">
            {!feedback ? (
              <button 
                ...
                disabled={!storyPartAnswer.trim() || isLoading}
              >
                {isLoading ? (
                  <>
