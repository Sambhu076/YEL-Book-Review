...
export default function GodAct1() {
  ...
  // MISSING VOICE STATE VARIABLES - ADDED HERE
  const [isSpeaking, setIsSpeaking] = useState(false);
  const [voiceEnabled, setVoiceEnabled] = useState(true);
  const [useElevenLabs, setUseElevenLabs] = useState(true);
  const [currentAudio, setCurrentAudio] = useState(null);
  
  const totalQuestions = 1; // Only one question in this component

  // ElevenLabs configuration - Fixed for browser environment
  ...
  // MISSING VOICE INITIALIZATION - ADDED HERE
  useEffect(() => {
    const loadVoices = () => {
  ...
  // FIXED SUBMIT ANSWER WITH VOICE FEEDBACK
  const submitAnswer = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        // VOICE FEEDBACK ADDED HERE - THIS WAS MISSING
        setTimeout(() => {
          let voiceText = "";
  ...
  const handleNextQuestion = async () => {
    if (titleAnswer.trim()) {
      // For question 1, check the answer first
      await submitAnswer();
    } else {
  ...
  // FIXED WITH STOP SPEAKING AND NAVIGATION
  const handleProceedToNext = () => {
    stopSpeaking(); // Stop any current speech
    // Navigate to GodAct2 instead of changing state
    navigate('/GodAct2');
  };

  // FIXED WITH STOP SPEAKING
          ...
          <div className="button-section">
            {!feedback ? (
              <button 
                ...
                disabled={!titleAnswer.trim() || isLoading}
              >
                {isLoading ? (
                  <>
