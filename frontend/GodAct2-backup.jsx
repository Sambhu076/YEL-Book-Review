...
export default function GodAct2() {
  ...
  const submitAnswer = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/check-question2/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleNextQuestion = async () => {
    if (authorAnswer.trim()) {
      // For question 2, check the answer first
      await submitAnswer();
    } else {
  ...
  const handleProceedToNext = () => {
    ...
    navigate('/GodAct3');
    // Example: navigate('/GodAct3') when you create it
    //alert('Question 3 will be implemented next!');
  };
